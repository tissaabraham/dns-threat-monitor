...
class Alert:
    """
    Represents a security alert triggered by the Detection Engine.
    Includes timestamp, affected domain, source IP, severity, and status tracking.
    """
    timestamp: datetime
    source_ip: str
    domain: str
    ...
    status: str = "new"  # new -> acknowledged -> resolved -> archived
...
class DnsEvent:
    """
    Represents a single DNS query/response event captured from network traffic.
    Includes query details, response information, and resolution results.
    """
    timestamp: datetime
    source_ip: str
    domain: str
